...
# =====================================================================
st.set_page_config(
    page_title="Aproar - Torre de Controle", page_icon="🚚", layout="wide"
)
...
def _normalizar_tabelas_relatorio(dados):
    ...
    for nome, tabela in itens:
        ...
        df = (
            ...
            else pd.DataFrame(tabela)
        )
        remover = [
            coluna
            for coluna in df.columns
            if str(coluna).strip().lower() in colunas_tecnicas
        ]
        df = df.drop(columns=remover, errors="ignore").rename(
            columns=nomes_amigaveis
        )
        for coluna in df.columns:
            if pd.api.types.is_datetime64_any_dtype(df[coluna]):
                df[coluna] = (
                    df[coluna].dt.strftime("%d/%m/%Y %H:%M").fillna("")
                )
        ...
        df = df.loc[
            ...
            [
                coluna
                for coluna in df.columns
                if not (df[coluna].astype(str).str.strip() == "").all()
            ],
        ]
        tabelas.append((str(nome), df.fillna("")))
    return tabelas or [
        ("Dados", pd.DataFrame({"Informação": ["Nenhum registro disponível."]}))
    ]
...
def _criar_resumo_analitico_relatorio(titulo, tabelas):
    ...
    def normalizar(valor):
        return re.sub(
            r"[^a-z0-9]+", " ", remover_acentos(str(valor or "")).lower()
        ).strip()
    ...
    def horario_medio(valores):
        minutos = [
            minuto
            for minuto in (minutos_horario(valor) for valor in valores)
    ...
    def adicionar(indicador, resultado, leitura=""):
        if resultado is None or (
            isinstance(resultado, float) and math.isnan(resultado)
        ):
            return
        indicadores.append(
            {
    ...
    principal = (
        max(tabelas_preenchidas, key=lambda item: len(item[1]))[1]
    ...
    if "rastreador" in titulo_norm:
        ...
        adicionar(
            ...
            total,
            "Quantidade de veículos com leitura atual.",
        )
        adicionar(
            "Veículos em movimento",
            len(em_movimento),
            (
                f"{numero_br(100 * len(em_movimento) / total, 1)}% da frota"
                " monitorada."
                if total
                else ""
            ),
        ...
        adicionar(
            "Veículos parados",
            total - len(em_movimento),
            "Veículos com velocidade igual a zero na última leitura.",
        )
        adicionar(
            "Velocidade média geral",
            f"{numero_br(velocidades.mean(), 1)} km/h"
            if len(velocidades)
            else None,
            "Inclui veículos parados.",
        )
        adicionar(
            "Velocidade média em movimento",
            f"{numero_br(em_movimento.mean(), 1)} km/h"
            if len(em_movimento)
            else None,
            "Considera somente velocidades acima de zero.",
        )
        inicio_col = localizar_coluna(principal, "Início da rota (hoje)")
        adicionar(
            "Horário médio de saída",
            horario_medio(principal[inicio_col]) if inicio_col else None,
            "Média das saídas registradas no dia.",
        )
        ...
        status = (
            principal[status_col].astype(str)
            if status_col
            else pd.Series([""] * total)
        )
        concluidas = int(
            status.str.contains(
                r"entregue|conclu", case=False, regex=True
            ).sum()
        )
        ...
        adicionar(
            "Demandas analisadas",
            total,
            "Volume total presente no relatório.",
        )
        adicionar(
            "Taxa de conclusão",
            f"{numero_br(100 * concluidas / total, 1)}%" if total else "0,0%",
            "Boa aderência"
            if total and concluidas / total >= 0.9
            else "Atenção às demandas ainda pendentes.",
        )
        ...
        adicionar(
            "Prioridade média",
            numero_br(prioridade.mean(), 2) if len(prioridade) else None,
            "Quanto maior, mais urgente é o conjunto de demandas.",
        )
        ...
        adicionar(
            ...
            f"{numero_br(coleta.mean(), 1)} min" if len(coleta) else None,
        )
        adicionar(
            "Tempo médio de entrega",
            f"{numero_br(entrega.mean(), 1)} min" if len(entrega) else None,
        )
        if len(coleta) and len(entrega):
            adicionar(
                "Tempo médio total por demanda",
                f"{numero_br(coleta.mean() + entrega.mean(), 1)} min",
                "Soma das médias de coleta e entrega.",
            )
        prazo_col = localizar_coluna(principal, "Prazo")
        if prazo_col:
            prazos = principal[prazo_col].astype(str)
            adicionar(
                "Demandas vencidas/atrasadas",
                int(
                    prazos.str.contains(
                        r"vencid|atrasad", case=False, regex=True
                    ).sum()
                ),
                "Prioridade de regularização.",
            )
        ...
        if parada_col:
            paradas_operacionais = {
                str(valor)
                for valor in paradas[parada_col]
        ...
        entregas = (
            paradas[
                paradas[acao_col]
                .astype(str)
                .str.contains("ENTREGAR", case=False, na=False)
            ].copy()
            if acao_col
            else paradas.copy()
        )
        status = (
            entregas[status_col].astype(str)
            if status_col
            else pd.Series([""] * len(entregas))
        )
        concluidas = int(
            status.str.contains(r"conclu|entregue", case=False, regex=True).sum()
        )
        ...
        adicionar(
            ...
            len(paradas_operacionais),
            "Não inclui almoço nem retorno à base.",
        )
        adicionar("Demandas previstas", total_demandas)
        adicionar(
            "Taxa de conclusão da rota",
            (
                f"{numero_br(100 * concluidas / total_demandas, 1)}%"
                if total_demandas
                else "0,0%"
            ),
            "Percentual de entregas previstas que tiveram baixa.",
        )
        adicionar("Demandas concluídas", concluidas)
        adicionar("Demandas pendentes", total_demandas - concluidas)
        adicionar(
            "Distância planejada",
            f"{numero_br(km_total, 1)} km" if km_total is not None else None,
        )
        adicionar(
            "Distância média por parada",
            (
                f"{numero_br(km_total / len(paradas_operacionais), 1)} km"
                if km_total is not None and paradas_operacionais
                else None
            ),
            "Indicador de dispersão da rota.",
        )
        if not resumo.empty:
            inicio_col = localizar_coluna(resumo, "Início")
            fim_col = localizar_coluna(resumo, "Término previsto")
            inicio = (
                minutos_horario(resumo.iloc[0][inicio_col])
                if inicio_col
                else None
            )
            fim = (
                minutos_horario(resumo.iloc[0][fim_col]) if fim_col else None
            )
            if inicio is not None and fim is not None:
                adicionar(
                    "Duração planejada da rota",
                    f"{numero_br((fim - inicio) / 60, 1)} h",
                    "Tempo estimado entre saída e retorno.",
                )
        ...
        adicionar(
            "Custo médio por KM",
            moeda_br(custo_km) if custo_km is not None else None,
            (
                "Dentro da referência de R$ 1,50/km."
                if custo_km is not None and custo_km <= 1.5
                else "Acima da referência de R$ 1,50/km."
        ...
        adicionar(
            ...
            moeda_br(combustivel),
            (
                f"{numero_br(100 * combustivel / custo_total, 1)}% do custo"
                " total."
                if custo_total
                else ""
            ),
        ...
        adicionar(
            ...
            moeda_br(manutencao),
            (
                f"{numero_br(100 * manutencao / custo_total, 1)}% do custo"
                " total."
                if custo_total
                else ""
            ),
        ...
        gastos = (
            pd.concat(
                [
                    df
                    for nome, df in tabelas
                    if normalizar(nome).startswith("gastos") and not df.empty
                ],
                ignore_index=True,
            )
            if any(
                normalizar(nome).startswith("gastos") and not df.empty
                for nome, df in tabelas
            )
            else pd.DataFrame()
        )
        ...
        adicionar(
            "Litros médios por abastecimento",
            f"{numero_br(litros.mean(), 1)} L" if len(litros) else None,
        )
        adicionar(
            "Custo médio por lançamento",
            moeda_br(numeros(gastos, "Total (R$)").dropna().mean())
            if len(numeros(gastos, "Total (R$)").dropna())
            else None,
        )
        ...
        inicios = next(
            (
                df
                for nome, df in tabelas
                if "inicios de rota" in normalizar(nome)
            ),
            pd.DataFrame(),
        )
        paradas = next(
            (
                df
                for nome, df in tabelas
                if "paradas rastreadas" in normalizar(nome)
            ),
            pd.DataFrame(),
        )
        abastecimentos = next(
            (
                df
                for nome, df in tabelas
                if "abastecimentos" in normalizar(nome)
            ),
            pd.DataFrame(),
        )
        quilometragens = next(
            (
                df
                for nome, df in tabelas
                if "quilometragens" in normalizar(nome)
            ),
            pd.DataFrame(),
        )
        saida_col = (
            localizar_coluna(inicios, "Hora Saída")
        ...
        adicionar(
            "Horário médio de saída",
            horario_medio(inicios[saida_col]) if saida_col else None,
            "Permite acompanhar a aderência ao início previsto.",
        )
        adicionar("Paradas rastreadas", len(paradas))
        chegada_col = (
            localizar_coluna(paradas, "Chegada")
            if not paradas.empty
            else None
        )
        saida_parada_col = (
            localizar_coluna(paradas, "Saída")
            if not paradas.empty
            else None
        )
        media_parada = (
            duracao_media(paradas[chegada_col], paradas[saida_parada_col])
            if chegada_col and saida_parada_col
            else None
        )
        adicionar(
            "Tempo médio por parada",
            f"{numero_br(media_parada, 1)} min"
            if media_parada is not None
            else None,
            "Considera paradas com chegada e saída registradas.",
        )
        locais_col = (
            localizar_coluna(paradas, "Local") if not paradas.empty else None
        )
        adicionar(
            "Locais distintos visitados",
            int(paradas[locais_col].astype(str).nunique())
            if locais_col
            else None,
        )
        litros = numeros(abastecimentos, "Litros").dropna()
        valor_litro = numeros(
            abastecimentos, "Valor por litro (R$)", "valor_litro"
        ).dropna()
        manutencao = numeros(
            abastecimentos, "Manutenção (R$)", "manutencao"
        ).fillna(0)
        combustivel_total = (
            float(
                (
                    numeros(abastecimentos, "Litros").fillna(0)
                    * numeros(
                        abastecimentos, "Valor por litro (R$)", "valor_litro"
                    ).fillna(0)
                ).sum()
            )
            if not abastecimentos.empty
            else 0
        )
        adicionar(
            "Litros médios por abastecimento",
        ...
        adicionar(
            ...
            moeda_br(valor_litro.mean()) if len(valor_litro) else None,
        )
        adicionar(
            "Custo total registrado",
            moeda_br(combustivel_total + manutencao.sum()),
            "Combustível calculado mais manutenções registradas.",
        )
        kms = numeros(quilometragens, "KM").dropna()
        adicionar(
            "KM total registrado",
            f"{numero_br(kms.sum(), 1)} km" if len(kms) else None,
        )
        adicionar(
            "KM médio por registro",
            f"{numero_br(kms.mean(), 1)} km" if len(kms) else None,
        )
        ...
        hora_col = localizar_coluna(
            principal, "Hora da conclusão", "hora_conclusao"
        )
        obras = (
            int(principal[obra_col].astype(str).nunique()) if obra_col else 0
        )
        destinos = (
            int(principal[destino_col].astype(str).nunique())
            if destino_col
            else 0
        )
        ...
        adicionar(
            "Média de entregas por obra",
            numero_br(total / obras, 2) if obras else None,
            "Ajuda a identificar concentração de atendimento.",
        )
        adicionar(
            "Média de entregas por destino",
            numero_br(total / destinos, 2) if destinos else None,
        )
        adicionar(
            "Horário médio das conclusões",
            horario_medio(principal[hora_col]) if hora_col else None,
        )
        if destino_col and total:
            frequencias = (
                principal[destino_col]
                .astype(str)
                .replace("", pd.NA)
                .dropna()
                .value_counts()
            ...
            if len(frequencias):
                adicionar(
                    "Destino com mais entregas",
                    f"{frequencias.index[0]} ({int(frequencias.iloc[0])})",
                    "Local de maior concentração no período.",
                )

    if not indicadores:
        adicionar(
            "Registros analisados",
            sum(len(df) for _, df in tabelas),
            "Total de linhas incluídas no relatório.",
        )
        numericas = (
            principal.select_dtypes(include="number")
        ...
        for coluna in numericas.columns[:4]:
            ...
            adicionar(
                f"Média de {coluna}",
                numero_br(serie.mean(), 2) if len(serie) else None,
            )

    return pd.DataFrame(
        indicadores, columns=["Indicador", "Resultado", "Leitura para análise"]
    )
...
def _dados_grafico_resumo(df):
    ...
    if (
        df is None
        or df.empty
        or "Indicador" not in df.columns
    ...
    valores = {
        normalizar(linha["Indicador"]): (
            str(linha["Indicador"]),
            valor_numerico(linha["Resultado"]),
        )
    ...
    comparacoes = [
        ...
        (
            "Saídas registradas",
            "Paradas rastreadas",
            "Atividade operacional registrada",
        ),
    ...
    for primeiro, segundo, titulo in comparacoes:
        ...
        if all(
            valor is not None and valor >= 0 for _, valor in itens
        ) and sum(valor for _, valor in itens) > 0:
            return titulo, itens
    return None, []
...
def _organizar_secoes_relatorio(titulo, tabelas):
    ...
    resumo = next(
        (
            df
            for nome, df in tabelas
            if normalizar(nome) == "resumo analitico"
        ),
        pd.DataFrame(),
    )
    dados = [
    ...
    for preferencia in preferencias:
        indice_principal = next(
            (
                i
                for i, (nome, _) in enumerate(dados)
                if preferencia in normalizar(nome)
            ),
            None,
        )
    ...
    if indice_principal is None:
        indice_principal = max(
            range(len(dados)), key=lambda i: len(dados[i][1])
        )

    principal = dados[indice_principal]
    ordenadas = [principal] + [
        item for i, item in enumerate(dados) if i != indice_principal
    ]
...
def _colunas_relevantes_pdf(df, limite=9):
    ...
    def pontuacao(coluna):
        ...
        return max(
            (pontos for chave, pontos in regras if chave in nome), default=50
        )

    posicoes = {coluna: indice for indice, coluna in enumerate(df.columns)}
    escolhidas = sorted(
        df.columns, key=lambda coluna: (-pontuacao(coluna), posicoes[coluna])
    )[:limite]
...
def _criar_csv_relatorio(tabelas):
    ...
    return consolidado.to_csv(
        index=False, sep=";", decimal=",", lineterminator="\n"
    ).encode("utf-8-sig")
...
def _criar_xlsx_basico(tabelas):
    ...
    for nome, df in tabelas:
        ...
        for numero_linha, valores in enumerate(linhas, start=1):
            celulas = "".join(
                celula_xml(
                    f"{coluna_excel(indice)}{numero_linha}",
                    valor,
                    1 if numero_linha == 1 else 0,
                )
    ...
    with zipfile.ZipFile(saida, "w", zipfile.ZIP_DEFLATED) as arquivo:
        ...
        sheets = "".join(
            f'<sheet name="{xml_escape(nome, {chr(34): "&quot;"})}"'
        ...
        arquivo.writestr(
            ...
            '<Relationships'
            ' xmlns="http://schemas.openxmlformats.org/package/2006/relationships">'
            f'{relacionamentos}<Relationship Id="rId{len(abas)+1}"'
        ...
        arquivo.writestr(
            ...
            '<styleSheet'
            ' xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">'
            '<fonts count="2"><font><sz val="11"/><name'
            ' val="Calibri"/></font><font><b/><color rgb="FFFFFFFF"/><sz'
            ' val="11"/><name val="Calibri"/></font></fonts>'
            '<fills count="3"><fill><patternFill'
            ' patternType="none"/></fill><fill><patternFill'
            ' patternType="gray125"/></fill><fill><patternFill'
            ' patternType="solid"><fgColor rgb="FF2563EB"/><bgColor'
...
def _criar_excel_relatorio(tabelas, titulo="Relatório Operacional"):
    def coluna_data_excel(coluna):
        ...
        return (
            "data" in nome
            or nome.startswith("dt.")
            or nome.startswith("dt ")
            or "limite d+4" in nome
            or "prazo final" in nome
        )
    ...
    def preparar_datas_excel(df):
        ...
        for coluna in preparado.columns:
            ...
            try:
                ...
                convertida = pd.to_datetime(
                    preparado[coluna], dayfirst=True, errors="coerce"
                )
            preenchidos = preparado[coluna].astype(str).str.strip() != ""
            if (
                preenchidos.any()
                and convertida[preenchidos].notna().mean() >= 0.8
            ):
    ...
    resumo_analitico, secoes_dados, _ = _organizar_secoes_relatorio(
        titulo, tabelas
    )
    tabelas_excel = list(secoes_dados)
    if not resumo_analitico.empty:
        tabelas_excel.insert(
            1 if tabelas_excel else 0, ("Resumo Analítico", resumo_analitico)
        )
    ...
    try:
        ...
        with pd.ExcelWriter(
            ...
            numero_fmt = workbook.add_format(
                {
                    "num_format": "#,##0.00",
                    "valign": "top",
                    "border": 1,
            ...
            identificador_fmt = workbook.add_format(
                {
                    "num_format": "@",
                    "valign": "top",
                    "text_wrap": True,
            ...
            cartao_rotulo_fmt = workbook.add_format(
                {
                    "bold": True,
                    "font_size": 8.5,
                    "font_color": "#FFFFFF",
            ...
            cartao_valor_fmt = workbook.add_format(
                {
                    "bold": True,
                    "font_size": 13,
                    "font_color": "#334155",
            ...
            cabecalho_tabela_fmt = workbook.add_format(
                {
                    "bold": True,
                    "font_size": 9,
                    "font_color": "#FFFFFF",
            ...
            for ordem_item, (nome, df_original) in enumerate(tabelas_excel):
                ...
                inicio_tabela = (
                    18 if eh_resumo_analitico else 6 if eh_principal else 3
                )
                df.to_excel(
                    escritor,
                    sheet_name=nome_aba,
                    index=False,
                    startrow=inicio_tabela,
                )
                ...
                if eh_resumo_analitico:
                    ...
                    worksheet.merge_range(
                        ...
                        1,
                        13,
                        f"Gerado em {datetime.now(FUSO_LOCAL).strftime('%d/%m/%Y às %H:%M')} • {len(df)} indicador(es)",
                        meta_fmt,
                    )
                    ...
                    worksheet.set_footer(
                        "&LAPROAR Engenharia&CResumo analítico&R Página &P"
                        " de &N"
                    )
                    ...
                    for indice, (_, indicador) in enumerate(
                        df.head(6).iterrows()
                    ):
                        linha = 3 + (indice // 2) * 4
                        ...
                        worksheet.merge_range(
                            ...
                            linha,
                            coluna + 2,
                            str(indicador.get("Indicador", "Indicador")),
                            cartao_rotulo_fmt,
                        )
                        worksheet.merge_range(
                            linha + 1,
                    ...
                    if dados_grafico:
                        ...
                        for posicao, (rotulo, valor) in enumerate(
                            dados_grafico, start=1
                        ):
                            indice_df = next(
                                (
                                    i
                                    for i, texto_indicador in enumerate(
                                        df["Indicador"].astype(str)
                                    )
                                    if texto_indicador == rotulo
                                ),
                                None,
                            )
                            linha_origem_excel = (
                                inicio_tabela + 2 + indice_df
                            ...
                            if linha_origem_excel:
                                worksheet.write_formula(
                                    linha_destino,
                                    15,
                                    f"=A{linha_origem_excel}",
                                    None,
                                    rotulo,
                                )
                                formula_valor = (
                                    '=IFERROR(NUMBERVALUE(SUBSTITUTE(SUBSTITUTE(B'
                                    f'{linha_origem_excel},"R$'
                                    ' ",""),"%",""),",","."),B'
                                    f"{linha_origem_excel})"
                                )
                                worksheet.write_formula(
                                    linha_destino,
                                    16,
                                ...
                                worksheet.write_number(
                                    linha_destino, 16, valor
                                )
                        worksheet.set_column(
                            15, 16, None, None, {"hidden": True}
                        )
                        ...
                        grafico.add_series(
                            {
                                ...
                                "categories": [
                                    ...
                                    linha_auxiliar + 1,
                                    15,
                                    linha_auxiliar + len(dados_grafico),
                                    15,
                                ],
                                "values": [
                                    nome_aba,
                        ...
                        grafico.set_legend(
                            {"position": "bottom", "font": {"size": 9}}
                        )
                        grafico.set_chartarea(
                            {"border": {"none": True}, "fill": {"color": "#FFFFFF"}}
                        )
                        grafico.set_plotarea(
                            {"border": {"none": True}, "fill": {"color": "#FFFFFF"}}
                        )
                    ...
                    worksheet.merge_range(
                        ...
                        16,
                        13,
                        "Leituras gerenciais e indicadores complementares",
                        subtitulo_fmt,
                    )
                    ...
                    if len(df):
                        worksheet.add_table(
                            inicio_tabela,
                            0,
                            inicio_tabela + len(df),
                            ultima_coluna,
                            {
                                "name": (
                                    f"Tabela_{len(usados)}_ResumoAnalitico"
                                ),
                                "style": "Table Style Light 9",
                                "columns": [
                                    {
                                        "header": str(coluna),
                                        "header_format": cabecalho_tabela_fmt,
                                    }
                                    for coluna in df.columns
                                ],
                            },
                    ...
                    for linha in range(
                        inicio_tabela + 1, inicio_tabela + 1 + len(df)
                    ):
                ...
                largura_modelo = max(
                    ultima_coluna, 5 if eh_principal else ultima_coluna
                )
                titulo_planilha = (
                    titulo.upper()
                    if eh_principal
                    else f"{titulo} - {nome}".upper()
                )
                if largura_modelo:
                    worksheet.merge_range(
                        0, 0, 0, largura_modelo, titulo_planilha, titulo_fmt
                    )
                else:
                ...
                worksheet.write(
                    1,
                    0,
                    f"Gerado em {datetime.now(FUSO_LOCAL).strftime('%d/%m/%Y às %H:%M')} • {len(df)} registro(s) • Seção: {nome}",
                    meta_fmt,
                )
                ...
                worksheet.set_margins(0.35, 0.35, 0.55, 0.55)
                worksheet.set_footer(
                    "&LAPROAR Engenharia&CRelatório operacional&R Página &P"
                    " de &N"
                )
                ...
                if eh_principal:
                    ...
                    for indice_kpi, (_, indicador) in enumerate(
                        resumo_analitico.head(6).iterrows()
                    ):
                        coluna_inicial = (
                            indice_kpi * total_colunas_painel
                        ) // 6
                        coluna_final = (
                            (indice_kpi + 1) * total_colunas_painel
                        ) // 6 - 1
                        rotulo = str(indicador.get("Indicador", "Indicador"))
                        resultado = str(indicador.get("Resultado", "-"))
                        if coluna_inicial == coluna_final:
                            worksheet.write(
                                3, coluna_inicial, rotulo, cartao_rotulo_fmt
                            )
                            worksheet.write(
                                4, coluna_inicial, resultado, cartao_valor_fmt
                            )
                        else:
                            worksheet.merge_range(
                                3,
                ...
                if len(df.columns):
                    if len(df):
                        worksheet.add_table(
                            ...
                            {
                                "name": (
                                    f"Tabela_{len(usados)}_{re.sub(r'[^A-Za-z0-9]', '', nome_aba)[:15] or 'Dados'}"
                                ),
                                "style": "Table Style Light 9",
                                "columns": [
                                    {
                    ...
                    for indice, coluna in enumerate(df.columns):
                        ...
                        if (
                            nome_aba == "Resumo Analítico"
                            and nome_coluna == "resultado"
                        ):
                            largura = 22
                        elif (
                            nome_aba == "Resumo Analítico"
                            and "indicador" in nome_coluna
                        ):
                            largura = 34
                        else:
                            largura = 52 if longa else min(32, max(12, maior + 2))
                        formato = (
                            identificador_fmt
                            if coluna_identificador(coluna)
                            else data_fmt
                            if coluna_data_excel(coluna)
                            else moeda_fmt
                            if "r$" in nome_coluna
                            or "custo" in nome_coluna
                            or "valor" in nome_coluna
                            or "manutencao" in nome_coluna
                            else numero_fmt
                            if nome_coluna
                            in {"km", "litros", "distancia (km)"}
                            else texto_fmt
                        )
                        worksheet.set_column(
                            indice, indice, largura, formato
                        )
                        if coluna_identificador(coluna):
                            for deslocamento, valor in enumerate(
                                df[coluna].tolist(),
                                start=inicio_tabela + 1,
                            ):
                                texto_valor = (
                    ...
                    for linha, valores in enumerate(
                        ...
                        maior_texto = max(
                            [len(valor) for valor in valores] + [0]
                        )
                    ...
                    for indice, coluna in enumerate(df.columns):
                        if (
                            "status" in remover_acentos(str(coluna)).lower()
                            and len(df)
                        ):
                            intervalo = (
                                inicio_tabela + 1,
                        ...
                        if (
                            "observacao"
                            in remover_acentos(str(coluna)).lower()
                            and len(df)
                        ):
                            intervalo = (
                                inicio_tabela + 1,
    ...
    try:
        ...
        with pd.ExcelWriter(saida, engine="openpyxl") as escritor:
            ...
            for ordem_item, (nome, df_original) in enumerate(tabelas_excel):
                ...
                for coluna_titulo in range(1, ultima_coluna + 1):
                    ws.cell(1, coluna_titulo).fill = PatternFill(
                        "solid", fgColor="08B7B7"
                    )
                tipo_linha = (
                    "indicador(es)"
                ...
                ws.cell(
                    2,
                    1,
                    f"Gerado em {datetime.now(FUSO_LOCAL).strftime('%d/%m/%Y às %H:%M')} • {len(df)} {tipo_linha}",
                )
                ...
                if eh_principal:
                    for indice_kpi, (_, indicador) in enumerate(
                        resumo_analitico.head(6).iterrows(), start=1
                    ):
                        ws.cell(
                            4,
                            indice_kpi,
                        ...
                        ws.cell(4, indice_kpi).font = Font(
                            bold=True, color="FFFFFF", size=9
                        )
                        ws.cell(4, indice_kpi).fill = PatternFill(
                            "solid", fgColor="4B4B4B"
                        )
                        ws.cell(5, indice_kpi).font = Font(
                            bold=True, color="334155", size=12
                        )
                        ws.cell(5, indice_kpi).fill = PatternFill(
                            "solid", fgColor="F3FAF3"
                        )
                        ws.cell(4, indice_kpi).alignment = Alignment(
                            horizontal="center",
                            vertical="center",
                            wrap_text=True,
                        )
                        ws.cell(5, indice_kpi).alignment = Alignment(
                            horizontal="center",
                            vertical="center",
                            wrap_text=True,
                        )
                ...
                ws.auto_filter.ref = (
                    f"A{linha_cabecalho}:{get_column_letter(max(1, len(df.columns)))}{max(linha_cabecalho, linha_cabecalho + len(df))}"
                )
                ...
                for indice, coluna in enumerate(df.columns, start=1):
                    ...
                    for linha in range(
                        linha_cabecalho + 1, linha_cabecalho + 1 + len(df)
                    ):
                        ws.cell(linha, indice).alignment = Alignment(
                        ...
                        if coluna_identificador(coluna):
                            ws.cell(linha, indice).number_format = "@"
                if nome_aba == "Resumo Analítico":
                    for linha in range(
                        linha_cabecalho + 1, linha_cabecalho + 1 + len(df)
                    ):
...
def _texto_pdf(valor):
    ...
    return "".join(
        chr(byte)
        if 32 <= byte <= 126 and chr(byte) not in "\\()"
        else f"\\{chr(byte)}"
        if chr(byte) in "\\()"
        else f"\\{byte:03o}"
...
def _criar_pdf_textual(titulo, tabelas):
        ...
        if contorno:
            comandos.extend(
                [
                    rgb(contorno, "RG"),
                    f"{espessura:.2f} w",
                    f"{x:.2f} {y:.2f} {largura:.2f} {altura:.2f} re B",
                ]
            )
        else:
    ...
    def escrever(
        ...
        comandos.extend(
            [
                rgb(cor, "rg"),
                f"BT /{fonte} {tamanho:.2f} Tf {x:.2f} {y:.2f} Td"
                f" ({_texto_pdf(texto)}) Tj ET",
            ]
        )
    ...
    def desenhar_secao(nome, df_original):
        ...
        escrever(
            comandos,
            f"{limpo(nome).upper()} - {len(df_original)} registro(s)",
            margem_x + 7,
            y - 14,
            9,
            True,
        ...
        if df.empty or not len(df.columns):
            escrever(
                comandos,
                "Nenhum registro disponível nesta seção.",
                margem_x + 6,
                y - 15,
                8,
                False,
        ...
        for coluna in df.columns:
            nome_coluna = remover_acentos(str(coluna)).lower()
            amostra = [len(str(coluna))] + [
                len(str(valor)) for valor in df[coluna].astype(str).head(80)
            ]
        ...
        for indice_registro, (_, registro) in enumerate(
            df.iterrows(), start=1
        ):
            linhas_celulas = [
                quebrar_texto(registro[coluna], largura - 8, 6.7, 240)
                for coluna, largura in zip(df.columns, larguras)
            ]
            altura_linha = max(
                20.0,
                min(
                    68.0,
                    8.3 * max(len(linhas) for linhas in linhas_celulas) + 7,
                ),
            )
            if y - altura_linha < limite_inferior:
                ...
                escrever(
                    comandos,
                    f"{limpo(nome).upper()} - CONTINUAÇÃO",
                    margem_x + 7,
                    y - 14,
                    9,
                    True,
            ...
            x = margem_x
            fundo_linha = (
                cores["fundo"] if indice_registro % 2 == 0 else cores["branco"]
            )
            for indice_coluna, (coluna, largura, linhas) in enumerate(
                zip(df.columns, larguras, linhas_celulas)
            ):
                ...
                if "status" in nome_coluna or "situacao" in nome_coluna:
                    ...
                    if re.search(r"conclu|entregue|dentro", status):
                        fundo_celula = cores["verde"]
                    elif re.search(r"fora|atras|vencid", status):
                        fundo_celula = cores["vermelho"]
                    elif re.search(r"pend|aguard", status):
                        fundo_celula = cores["amarelo"]
                retangulo(
                    comandos,
                    x,
                    y - altura_linha,
    ...
    if not resumo_analitico.empty:
        leituras = resumo_analitico[
            resumo_analitico["Leitura para análise"].astype(str).str.strip()
            != ""
        ]
        complementares = resumo_analitico.iloc[6:]
        analises = pd.concat(
            [leituras, complementares], ignore_index=True
        ).drop_duplicates(subset=["Indicador"])
        if not analises.empty:
            desenhar_secao(
                "Análises complementares",
                analises[
                    ["Indicador", "Resultado", "Leitura para análise"]
                ].rename(columns={"Leitura para análise": "Interpretação"}),
            )
    ...
    objetos = [
        ...
        (
            b"<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold /Encoding"
            b" /WinAnsiEncoding >>"
        ),
    ]
    ids_paginas = []
    ...
    for numero_pagina, comandos_pagina in enumerate(paginas, start=1):
        moldura = []
        retangulo(
            moldura, 0, altura_pagina - 28, largura_pagina, 28, cores["cinza"]
        )
        escrever(
            moldura,
            "APROAR ENGENHARIA",
        ...
        moldura.extend(
            [
                rgb(cores["borda"], "RG"),
                "0.5 w",
                f"{margem_x:.2f} 30 m {largura_pagina - margem_x:.2f} 30 l S",
            ]
        )
        escrever(
            moldura,
        ...
        objetos.append(
            f"<< /Type /Page /Parent 2 0 R /MediaBox [0 0 {largura_pagina:.0f} {altura_pagina:.0f}] /Resources << /Font << /F1 3 0 R /F2 4 0 R >> >> /Contents {id_conteudo} 0 R >>".encode(
                "ascii"
            )
        )
        objetos.append(
    ...
    objetos[1] = (
        f"<< /Type /Pages /Kids [{' '.join(f'{i} 0 R' for i in ids_paginas)}] /Count {len(ids_paginas)} >>".encode(
            "ascii"
        )
    )
...
def _criar_pdf_relatorio(titulo, tabelas):
    ...
    def cabecalho_rodape(canvas, doc):
        ...
        canvas.drawRightString(
            largura - 12 * mm, altura - 6.5 * mm, "Torre de Controle Logístico"
        )
        ...
        canvas.drawRightString(
            largura - 12 * mm, 5 * mm, f"Página {doc.page}"
        )
        canvas.restoreState()

    faixa_titulo = Table(
        [[Paragraph(html_escape(titulo_limpo.upper()), estilo_titulo)]],
        colWidths=[largura_util],
        rowHeights=[26],
    )
    faixa_titulo.setStyle(
        TableStyle(
            [
    ...
    if not resumo_analitico.empty:
        ...
        dados_kpi = [
            [
                paragrafo(
                    linha.get("Indicador", "Indicador"),
                    estilo_kpi_rotulo,
                    90,
                )
                for _, linha in kpis.iterrows()
            ],
            [
                paragrafo(
                    linha.get("Resultado", "-"), estilo_kpi_valor, 80
                )
                for _, linha in kpis.iterrows()
            ],
        ]
        while len(dados_kpi[0]) < 6:
            ...
            dados_kpi[1].append("")
        painel_kpi = Table(
            dados_kpi, colWidths=[largura_util / 6] * 6, rowHeights=[28, 32]
        )
        painel_kpi.setStyle(
            TableStyle(
                [
    ...
    for nome, df_original in secoes_dados:
        elementos.append(
            Paragraph(
                f"{html_escape(texto_pdf_limpo(nome).upper())} &nbsp; - &nbsp;"
                f" {len(df_original)} registro(s)",
                estilo_secao,
            )
        )
        if df_original.empty or not len(df_original.columns):
            elementos.append(
                Paragraph(
                    "Nenhum registro disponível nesta seção.", estilo_vazio
                )
            )
            continue
        ...
        dados_tabela = [
            [paragrafo(coluna, estilo_cabecalho, 70) for coluna in df.columns]
        ]
        dados_tabela.extend(
            [
                [paragrafo(valor, estilo_celula) for valor in linha]
                for linha in df.astype(str).values.tolist()
            ]
        )
        tabela = Table(
            dados_tabela, colWidths=larguras, repeatRows=1, hAlign="LEFT"
        )
        comandos = [
        ...
        for indice_linha in range(1, len(dados_tabela)):
            comandos.append(
                (
                    ...
                    (0, indice_linha),
                    (-1, indice_linha),
                    cinza_fundo if indice_linha % 2 == 0 else colors.white,
                )
            )
        for indice_coluna, coluna in enumerate(df.columns):
            ...
            if "status" not in nome_coluna and "situacao" not in nome_coluna:
                continue
            for indice_linha, valor in enumerate(
                df[coluna].astype(str), start=1
            ):
                texto_status = remover_acentos(valor).lower()
                cor = (
                    verde_claro
                    if re.search(r"conclu|entregue|dentro", texto_status)
                    else vermelho_claro
                    if re.search(r"fora|atras|vencid", texto_status)
                    else amarelo_claro
                    if re.search(r"pend|aguard", texto_status)
                    else None
                )
                if cor:
                    comandos.append(
    ...
    if not resumo_analitico.empty:
        ...
        if not analises.empty:
            elementos.append(
                Paragraph("ANÁLISES COMPLEMENTARES", estilo_secao)
            )
            dados_analise = [
                [
            ...
            dados_analise.extend(
                [
                    [
                        paragrafo(
                            linha.get("Indicador", ""), estilo_celula
                        ),
                        paragrafo(
                            linha.get("Resultado", ""), estilo_celula
                        ),
                        paragrafo(
                            linha.get("Leitura para análise", ""),
                            estilo_celula,
                        ),
                    ]
    ...
    documento.build(
        elementos, onFirstPage=cabecalho_rodape, onLaterPages=cabecalho_rodape
    )
    return saida.getvalue()
...
def _conteudo_exportador(titulo, dados, nome_arquivo, chave):
    ...
    formato = col_formato.selectbox(
        "Formato", ["PDF", "CSV", "Excel"], key=f"formato_relatorio_{chave}"
    )
    ...
    if formato == "CSV":
        arquivo, extensao, mime = (
            _criar_csv_relatorio(tabelas),
            "csv",
            "text/csv",
        )
    elif formato == "Excel":
        arquivo, extensao, mime = (
            _criar_excel_relatorio(tabelas, titulo),
            "xlsx",
            (
        ...
        arquivo, extensao, mime = (
            _criar_pdf_relatorio(titulo, tabelas),
            "pdf",
            "application/pdf",
        )
...
def renderizar_detalhes_fechamento(veiculo, gastos, quilometragem, chave):
    mostrar = st.checkbox(
        f"Ver lançamentos da {veiculo}", key=f"mostrar_fechamento_{chave}"
    )
    if not mostrar:
        return
    ...
    if quilometragem.empty:
        ...
        st.dataframe(
            quilometragem, use_container_width=True, hide_index=True
        )
    ...
    for step in route_steps:
        ...
        if step.get("type") == "return":
            linhas.append(
                {
                    "Parada": "Retorno",
                    "Local": step.get("destino", ""),
                    "Chegada": step.get(
                        "dyn_chegada", step.get("chegada", "")
                    ),
                    "Saída": step.get("dyn_saida", step.get("saida", "")),
                    "Ação": "RETORNO",
                    "Distância (km)": step.get("dist", 0),
                }
            )
            continue
        numero_parada += 1
            ...
            linhas.append(
                {
                    ...
                    "Saída": step.get("dyn_saida", step.get("saida", "")),
                    "Distância (km)": round(
                        float(step.get("dist", 0) or 0), 2
                    ),
                    "Ação": acao,
                    "Obra": tarefa.get("Obra", ""),
                    "Materiais": tarefa.get("Materiais", ""),
                    "Status": (
                        f"Concluída às {concluidos[card_id]}"
                        if card_id in concluidos
                        else "Pendente"
                    ),
                }
            )
...
def obter_hora_inicio_rota(data_rota):
    ...
    inicio_davi = fetch_one(
        "SELECT hora_inicio FROM inicio_movimento WHERE placa=:placa AND"
        " data=:data",
    ...
    return (
        str(primeiro_inicio[0])
        if primeiro_inicio and primeiro_inicio[0]
        else "07:30"
    )
...
def aplicar_tempos_dinamicos(route_steps, dict_concluidos, start_time_str):
    ...
    current_min = (
        parse_time_to_mins(start_time_str)
        if start_time_str
        else (7 * 60 + 0)
    )
    ...
    for step in route_steps:
        ...
        for a, t in step.get("actions", []):
            ...
            if card_id in dict_concluidos:
                concluded_times.append(
                    parse_time_to_mins(dict_concluidos[card_id])
                )
            else:
                has_pending = True
...
def renderizar_banner_eta(hora_atual_str, nova_previsao_str, final_dyn_min):
    ...
    cor_previsao = (
        "#16a34a"
        if final_dyn_min <= (17 * 60)
        else "#f59e0b"
        if final_dyn_min <= (17 * 60 + 30)
        else "#ef4444"
    )
...
def filtrar_checkins_da_rota(route_steps, checkins):
    ...
    return {
        ...
        and str(route_steps[indice].get("destino", ""))
        == dados.get("destino", "")
    }
...
if modo_url == "true":
    ...
    st.markdown(
        "<h2 style='text-align: center; color: #e4e8f4; margin-bottom: 0;'>📱"
        " App do Motorista</h2>",
        unsafe_allow_html=True,
    )
    st.caption(
        "<div style='text-align:center; font-size: 14px; margin-bottom: 15px;"
        f" color: #8da0b8;'>Rota Oficial de: <b>{DATA_REF_ROTA_STR}</b></div>",
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        res = fetch_one(
            "SELECT json_route, json_locais, json_geometria, json_enderecos,"
            " total_km FROM rota_ativa WHERE id = 1 AND data_rota = :data",
            {"data": DATA_REF_ROTA_STR},
        )
        df_mobile = get_df(
            "SELECT id, hora_conclusao FROM historico_concluidos WHERE"
            " data_conclusao = :data",
            {"data": DATA_REF_ROTA_STR},
        )
        dict_concluidos_mobile = dict(
            zip(df_mobile["id"].astype(str), df_mobile["hora_conclusao"])
        )
        hora_inicio_real = obter_hora_inicio_rota(DATA_REF_ROTA_STR)
    except:
        res, dict_concluidos_mobile, hora_inicio_real = None, {}, "07:30"

    if not res:
        st.info(
            "Nenhuma rota foi liberada pela Torre de Controle para hoje ainda."
            " Aguarde a central calcular e tente atualizar a tela."
        )
        st.stop()
    ...
    try:
        dict_checkins_mobile = filtrar_checkins_da_rota(
            route_steps, carregar_checkins_davi(DATA_REF_ROTA_STR)
        )
    except Exception:
        dict_checkins_mobile = {}
        erro_checkin_mobile = (
            "Não foi possível carregar as marcações compartilhadas agora."
        )

    if erro_checkin_mobile:
        st.error(erro_checkin_mobile)

    route_steps, final_dyn_min = aplicar_tempos_dinamicos(
        route_steps, dict_concluidos_mobile, hora_inicio_real
    )
    ...
    st.markdown(
        "<h4 style='color: #e4e8f4; margin-bottom:4px;'>Roteiro Passo a Passo"
        f" ({total_km:.1f} km)</h4>",
        unsafe_allow_html=True,
    )
    ...
    for i, step in enumerate(route_steps):
        ...
        if tipo_step == "lunch":
            ...
            meta_card = (
                "Horário previsto:"
                f" {html_escape(str(step.get('dyn_chegada', '12:00')))} às"
                f" {html_escape(str(step.get('dyn_saida', '13:00')))}"
            )
            corpo_acoes = (
                "<div class='mensagem-etapa'>Pausa programada para descanso e"
                " alimentação.</div>"
            )
            ...
            meta_card = (
                "Chegada prevista:"
                f" {html_escape(str(step.get('dyn_chegada', '')))}"
            )
            corpo_acoes = (
                "<div class='mensagem-etapa'>Última etapa do roteiro. Retorne"
                " para a base indicada.</div>"
            )
        else:
            ...
            if str(endereco_db).startswith("http"):
                ...
                link_gps = (
                    ...
                    + urllib.parse.quote(str(endereco_db))
                )
            ...
            if is_start:
                ...
                meta_card = (
                    f"Base: {html_escape(str(step.get('chegada', '')))} às"
                    f" {html_escape(str(step.get('saida', '')))}"
                )
            else:
                selo = f"PARADA {numero_parada_mobile}"
                titulo_card = f"📍 {html_escape(destino_step)}"
                meta_card = (
                    f"Trecho: {float(step.get('dist', 0) or 0):.1f} km | Base:"
                    f" {html_escape(str(step.get('chegada', '')))} às"
                    f" {html_escape(str(step.get('saida', '')))}"
                )

            status_tempo = (
                "<span class='status concluido'>✅ Concluído às"
                f" {html_escape(str(step.get('dyn_saida', '')))}</span>"
                if step.get("is_concluded")
                else "<span class='status pendente'>⏳ Previsão:"
                f" {html_escape(str(step.get('dyn_chegada', '')))} às"
                f" {html_escape(str(step.get('dyn_saida', '')))}</span>"
            )
            blocos_acao = []
            for acao, tarefa in step.get("actions", []):
                ...
                classe_acao, icone = (
                    ("coleta", "📦") if eh_coleta else ("entrega", "📬")
                )
                card_id = str(tarefa.get("id", ""))
                concluido = (
                    "<div class='baixa'>✅ Baixa às"
                    f" {html_escape(str(dict_concluidos_mobile[card_id]))}</div>"
                    if card_id in dict_concluidos_mobile
                    else ""
                )
                blocos_acao.append(
                    f"<div class='acao {classe_acao}'><div"
                    f" class='acao-titulo'>{icone}"
                    f" {html_escape(str(acao))}</div><div"
                    " class='materiais'>"
                    f"{html_escape(str(tarefa.get('Materiais', '')))}</div><div"
                    " class='obra'>Obra:"
                    f" {html_escape(str(tarefa.get('Obra', '')))}</div>{concluido}</div>"
                )
            corpo_acoes = status_tempo + (
                "".join(blocos_acao)
                if blocos_acao
                else "<div class='mensagem-etapa'>Nenhuma movimentação"
                " cadastrada nesta etapa.</div>"
            )
            rotulo_lembrete = "preparação" if is_start else "parada"
            checkin_etapa = dict_checkins_mobile.get(i)
            etapa_marcada = bool(checkin_etapa) or bool(
                step.get("is_concluded")
            )
            if step.get("is_concluded"):
                botao_feito = (
                    "<button class='marcar-feita ativa' data-feita='1'"
                    " disabled>✅ Concluída no sistema</button>"
                )
            else:
                novo_estado = "0" if checkin_etapa else "1"
                classe_marcacao = " ativa" if checkin_etapa else ""
                texto_marcacao = (
                    f"✅ Marcada às {html_escape(checkin_etapa['hora'])} • toque"
                    " para desfazer"
                    if checkin_etapa
                    else f"☐ Marcar {rotulo_lembrete} como feita"
                )
                link_marcacao = html_escape(
                    f"?davi=true&etapa={i}&feito={novo_estado}", quote=True
                )
                botao_feito = (
                    f"<a class='marcar-feita{classe_marcacao}'"
                    f" data-feita='{'1' if checkin_etapa else '0'}'"
                    f" href='{link_marcacao}' target='_top'"
                    f" onclick='prepararEnvio(this)'>{texto_marcacao}</a>"
                )
            if not is_start:
                if link_gps:
                    botao_gps = (
                        f"<a class='gps' href='{html_escape(link_gps, quote=True)}'"
                        " target='_blank' rel='noopener'>🧭 ABRIR GPS DA"
                        f" PARADA {numero_parada_mobile}</a>"
                    )
                numero_parada_mobile += 1

        rodape_card = (
            f"<div class='rodape-card'>{botao_feito}{botao_gps}</div>"
            if botao_feito or botao_gps
            else ""
        )
        cartoes_mobile.append(
            f"<article class='cartao {classe_card}{' feita' if etapa_marcada else ''}'><div"
            f" class='topo-card'><span class='selo'>{html_escape(str(selo))}</span><h2>{titulo_card}</h2><div"
            f" class='meta'>{meta_card}</div></div><div"
            f" class='conteudo-card'>{corpo_acoes}</div>{rodape_card}</article>"
        )
    ...
    if cartoes_mobile:
        ...
        """.replace(
            "__CARTOES__", "".join(cartoes_mobile)
        ).replace(
            "__TOTAL__", str(len(cartoes_mobile))
        )
    ...
    m_mobile = folium.Map(
        location=[-3.7319, -38.5267], zoom_start=12, tiles="OpenStreetMap"
    )
    path_points_mobile = []
    offsets_dict_mobile = {}
    ...
    for i, step in enumerate(route_steps):
        if step.get("destino") and step["destino"] in locais_dict:
            if step["type"] in ["lunch", "return"]:
                continue
            if i == 0 and step["destino"] == p_saida:
                continue
            ...
            folium.Marker(
                ...
                icon=folium.DivIcon(
                    html=(
                        f'<div style="background: {fundo_marcador}; color:'
                        " white; border: 2px solid white; border-radius: 50%;"
                        " width: 30px; height: 30px; display: flex;"
                        " justify-content: center; align-items: center;"
                        " font-weight: bold; box-shadow: 2px 2px 5px"
                        f' rgba(0,0,0,0.5); font-size: 14px;">{p_num_mapa}</div>'
                    )
                ),
            ).add_to(m_mobile)
            p_num_mapa += 1
    ...
    if len(geometria_rota) > 1:
        folium.PolyLine(
            geometria_rota, color="#2563eb", weight=5, opacity=0.85
        ).add_to(m_mobile)
    if len(path_points_mobile) > 1:
        m_mobile.fit_bounds(path_points_mobile, padding=(30, 30), max_zoom=14)
    if p_saida in locais_dict:
        folium.Marker(
            [path_points_mobile[0][0], path_points_mobile[0][1]],
            popup=folium.Popup(
                f"<b>Saída: {html_escape(str(p_saida))}</b>", max_width=280
            ),
            z_index_offset=1000,
            icon=folium.DivIcon(
                html=(
                    '<div style="background: linear-gradient(135deg, #2563eb,'
                    " #1d4ed8); color: white; border: 3px solid white;"
                    " border-radius: 50%; width: 34px; height: 34px; display:"
                    " flex; justify-content: center; align-items: center;"
                    " box-shadow: 2px 2px 7px rgba(0,0,0,0.6); font-size:"
                    ' 16px;">🏁</div>'
                )
            ),
        ).add_to(m_mobile)
    ...
    st_folium(
        m_mobile, height=400, use_container_width=True, returned_objects=[]
    )
    st.markdown(
        "<div style='text-align: center; font-size: 13px; margin-top: 5px;"
        " color: #8da0b8;'><b>Legenda:</b> 🟡 Coleta | 🟢 Entrega | 🏁 Início |"
        " 🟡🟢 Ambos</div>",
        unsafe_allow_html=True,
    )
    st.divider()
...
LOCAL_BASE_ENDERECO = (
    "Rua Professor Mário Rocha, 84 - Joaquim Távora, Fortaleza - CE, 60120-200"
)
...
ENDERECOS_PADRAO = [
    ...
    (
        "SESI BARRA DO CEARÁ",
        "Rua Florencio de Alencar, 900 - Barra do Ceará, Fortaleza - CE",
    ),
    (
    ...
    (
        "SESI ALBANO FRANCO",
        "Av. Sen. Virgílio Távora, 1395 - Distrito Industrial I, Maracanaú - CE",
    ),
    (
    ...
    (
        "SESI SOBRAL",
        "Av. Dr. José Arimathéa Monte e Silva, 1003 - Junco, Sobral - CE",
    ),
    ("ESCRITÓRIO", LOCAL_BASE_ENDERECO),
    ("ALMOXARIFADO", LOCAL_BASE_ENDERECO),
    (
        "ESPAÇO SMART",
        "BR-116, 9370 - Barroso, Fortaleza - CE, 60862-735",
    ),
    ("ALDEOTA", "Rua Dr. José Lourenço, 1990 - Aldeota, Fortaleza - CE"),
    (
        "EDSON QUEIROZ",
        "Av. Dr. Valmir Pontes, 675 - Edson Queiroz, Fortaleza - CE",
    ),
    ...
    (
        "HORIZONTE",
        "R. Raimunda Pontes - Planalto Horizonte, Horizonte - CE",
    ),
    ("SEBRAE", "Avenida Monsenhor Tabosa, 777 - Meireles, Fortaleza - CE"),
    (
        "LECI FERRAGENS",
        "Rua Gen. Clarindo de Queiroz, 1668 - Centro, Fortaleza - CE",
    ),
...
def inicializar_bd():
    ...
    queries = [
        (
            "CREATE TABLE IF NOT EXISTS locais (apelido TEXT PRIMARY KEY,"
            " endereco TEXT, lat REAL, lon REAL)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS locais_removidos (apelido TEXT PRIMARY"
            " KEY)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS config_frota (id SERIAL PRIMARY KEY,"
            " consumo REAL, preco_gasolina REAL)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS abastecimentos (id SERIAL PRIMARY KEY,"
            " data TEXT, litros REAL, valor_litro REAL, manutencao REAL, obs"
            " TEXT, veiculo TEXT DEFAULT 'Strada')"
        ),
        (
            "CREATE TABLE IF NOT EXISTS registro_km (id SERIAL PRIMARY KEY,"
            " data TEXT, km REAL, obs TEXT, veiculo TEXT DEFAULT 'Strada')"
        ),
        (
            "CREATE TABLE IF NOT EXISTS historico_concluidos (id TEXT PRIMARY"
            " KEY, obra TEXT, origem TEXT, destino TEXT, materiais TEXT,"
            " data_conclusao TEXT, hora_conclusao TEXT)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS rastreio_paradas (id SERIAL PRIMARY"
            " KEY, data TEXT, placa TEXT, local TEXT, hora_chegada TEXT,"
            " hora_saida TEXT)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS inicio_movimento (placa TEXT, data"
            " TEXT, hora_inicio TEXT, PRIMARY KEY(placa, data))"
        ),
        (
            "CREATE TABLE IF NOT EXISTS webhooks_teams (setor TEXT PRIMARY KEY,"
            " url TEXT)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS config_trello (id SERIAL PRIMARY KEY,"
            " api_key TEXT, token TEXT, id_lista_concluida TEXT)"
        ),
        (
            "CREATE TABLE IF NOT EXISTS trello_cache (id SMALLINT PRIMARY KEY,"
            " dados JSONB NOT NULL DEFAULT '{}'::jsonb, atualizado_em"
            " TIMESTAMPTZ NOT NULL DEFAULT NOW())"
        ),
        (
            "CREATE TABLE IF NOT EXISTS rota_ativa (id SERIAL PRIMARY KEY,"
            " data_rota TEXT, json_route TEXT, json_locais TEXT, json_geometria"
            " TEXT, json_enderecos TEXT, total_km REAL)"
        ),
        SQL_TABELA_CHECKINS_DAVI,
        ...
        (
            "CREATE INDEX IF NOT EXISTS idx_historico_concluidos_data ON"
            " historico_concluidos (data_conclusao)"
        ),
        (
            "CREATE INDEX IF NOT EXISTS idx_inicio_movimento_data ON"
            " inicio_movimento (data)"
        ),
        (
            "CREATE INDEX IF NOT EXISTS idx_rastreio_paradas_data_placa ON"
            " rastreio_paradas (data, placa)"
        ),
        (
            "CREATE INDEX IF NOT EXISTS idx_rastreio_paradas_abertas ON"
            " rastreio_paradas (data, placa, hora_saida)"
        ),
    ]
    ...
    with conn_db.session as s:
        ...
        s.execute(
            text(
                "INSERT INTO config_frota (id, consumo, preco_gasolina) VALUES"
                " (1, 11.5, 5.90) ON CONFLICT (id) DO NOTHING"
            )
        )
        s.execute(
            text(
                "INSERT INTO webhooks_teams (setor, url) VALUES ('Geral /"
                " Logística', '') ON CONFLICT (setor) DO NOTHING"
            )
        )
        for sup in set(SUPERVISORES_MAP.values()):
            s.execute(
                text(
                    "INSERT INTO webhooks_teams (setor, url) VALUES (:sup, '')"
        ...
        locais_existentes = {
            ...
            for row in s.execute(
                text("SELECT apelido, endereco FROM locais")
            ).fetchall()
        }
        locais_removidos = {
            row[0]
            for row in s.execute(
                text("SELECT apelido FROM locais_removidos")
            ).fetchall()
        }
        ...
        for apelido, end in ENDERECOS_PADRAO:
            if apelido in locais_existentes:
                if locais_existentes[apelido] != end:
                    s.execute(
                        text(
                            "UPDATE locais SET endereco = :end, lat = NULL, lon"
                            " = NULL WHERE apelido = :apelido"
                        ),
                        {"end": end, "apelido": apelido},
                    )
            elif apelido not in locais_removidos:
                s.execute(
                    text(
                        "INSERT INTO locais (apelido, endereco) VALUES"
                        " (:apelido, :end)"
                    ),
                    {"apelido": apelido, "end": end},
                )

        s.execute(
            text(
                "DELETE FROM locais WHERE UPPER(TRIM(apelido)) ="
                " 'DESCONHECIDO'"
            )
        )
        for alias in ALIASES_LOCAL_BASE:
            s.execute(
                text(
                    "INSERT INTO locais (apelido, endereco, lat, lon) VALUES"
                    " (:alias, :end, :lat, :lon) ON CONFLICT (apelido) DO"
                    " UPDATE SET endereco=EXCLUDED.endereco,"
                    " lat=EXCLUDED.lat, lon=EXCLUDED.lon"
                ),
                {
                    "alias": alias,
                    "end": LOCAL_BASE_ENDERECO,
                    "lat": LOCAL_BASE_COORDS[0],
                    "lon": LOCAL_BASE_COORDS[1],
                },
            )
...
try:
    inicializar_bd()
    if (
        "rota_gerada" not in st.session_state
        or not st.session_state.get("rota_gerada")
    ):
        res_rota = fetch_one(
            "SELECT json_route, json_locais, json_geometria, json_enderecos,"
            " total_km, fonte_matriz, horario_matriz FROM rota_ativa WHERE id ="
            " 1 AND data_rota = :data",
            {"data": DATA_REF_ROTA_STR},
        )
        if res_rota:
            ...
            st.session_state["fonte_matriz_rota"] = (
                res_rota[5] or "OSRM — malha viária sem trânsito ao vivo"
            )
            st.session_state["horario_matriz_rota"] = res_rota[6] or ""
            if st.session_state["route_steps"]:
                st.session_state["p_saida"] = st.session_state["route_steps"][
                    0
                ]["destino"]
                h, m = map(
                    int,
                    st.session_state["route_steps"][-1]["saida"].split(":"),
                )
    ...
    st.error(
        "⚠️ Atenção: Não foi possível conectar ao Banco de Dados Supabase."
        " Verifique se as credenciais estão corretas nos Secrets do Streamlit."
    )
    st.stop()
...
def ler_cache_trello_supabase():
    ...
    try:
        ...
        if (
            ...
            and isinstance(dados.get("cards"), list)
            and isinstance(dados.get("lists"), list)
        ):
            return dados
    except Exception:
        pass
...
def obter_webhook_teams(setor, supervisor=None, obra=""):
    ...
    try:
        registro = fetch_one(
            "SELECT url FROM webhooks_teams WHERE setor = :setor",
            {"setor": supervisor or setor},
        )
...
def buscar_coordenadas(endereco):
    ...
    if match_coords:
        ...
        if is_in_ceara(lat, lon):
            return lat, lon
    try:
        url_arcgis = (
            "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?f=json&singleLine="
            + urllib.parse.quote(endereco_limpo + ", Ceará, Brasil")
            + "&maxLocations=1"
        )
        req = urllib.request.Request(
            url_arcgis, headers={"User-Agent": "AproarLogisticsWeb/1.0"}
        )
        with urllib.request.urlopen(req, timeout=5) as response:
            data = json.loads(response.read())
            if data.get("candidates"):
                lat, lon = (
                    float(data["candidates"][0]["location"]["y"]),
                    float(data["candidates"][0]["location"]["x"]),
                )
                if is_in_ceara(lat, lon):
                    return lat, lon
    except:
...
def normalizar_local(nome):
    ...
    n = nome.upper().strip()
    if "MARACANAU" in n:
        n = n.replace("MARACANAU", "MARACANAÚ")
    if "ESCRITORIO" in n:
        n = n.replace("ESCRITORIO", "ESCRITÓRIO")
    if "DEPÓSITO" in n:
        n = n.replace("DEPÓSITO", "DEPOSITO")
    if "ESPACO" in n:
        n = n.replace("ESPACO", "ESPAÇO")
    return n
...
def garantir_gps_local_base():
    ...
    for alias in ("ESCRITÓRIO", "ALMOXARIFADO"):
        registro = fetch_one(
            "SELECT lat, lon FROM locais WHERE apelido = :alias",
            {"alias": alias},
        )
    ...
    if coordenadas is not None:
        for alias in ALIASES_LOCAL_BASE:
            execute_db(
                "INSERT INTO locais (apelido, endereco, lat, lon) VALUES"
                " (:alias, :end, :lat, :lon) ON CONFLICT (apelido) DO UPDATE"
                " SET endereco=EXCLUDED.endereco, lat=EXCLUDED.lat,"
                " lon=EXCLUDED.lon",
                {
                    "alias": alias,
                    "end": LOCAL_BASE_ENDERECO,
...
def calcular_matriz_google_trafego(coords, horario_partida):
    ...
    partida = (
        horario_partida
        if horario_partida and horario_partida > agora_seguro
        else agora_seguro
    )
    partida_rfc3339 = (
        partida.astimezone(timezone.utc).isoformat().replace("+00:00", "Z")
    )
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(4, len(blocos))) as executor:
            futuros = [
                executor.submit(consultar_bloco, indices) for indices in blocos
            ]
            for futuro in as_completed(futuros):
                ...
                for elemento in elementos:
                    if elemento.get("condition") != "ROUTE_EXISTS":
                        continue
                    origem_global = indices_origem[
                        int(elemento.get("originIndex", 0))
                    ]
                    destino_global = int(
                        elemento.get("destinationIndex", 0)
                    )
                    duracao_min = _duracao_google_minutos(
                        elemento.get("duration")
                    )
                    distancia_m = elemento.get("distanceMeters")
                    if duracao_min is None or distancia_m is None:
                        continue
                    distancias[origem_global][destino_global] = (
                        float(distancia_m) / 1000.0
                    )
...
def calcular_matriz_tomtom_trafego(coords, horario_partida):
    ...
    try:
        ...
        distancias = [
            [None for _ in range(quantidade)] for _ in range(quantidade)
        ]
        duracoes = [
            [None for _ in range(quantidade)] for _ in range(quantidade)
        ]
        ...
        for celula in dados:
            ...
            if (
                0 <= origem < quantidade
                and 0 <= destino < quantidade
                and distancia_m is not None
                and duracao_s is not None
            ):
...
def calcular_matriz_rotas(coords, horario_partida=None):
    ...
    if matriz_tomtom:
        return (
            matriz_tomtom[0],
            matriz_tomtom[1],
            "TomTom Routing — trânsito ao vivo e histórico (gratuito)",
        )
    ...
    if matriz_google:
        return (
            matriz_google[0],
            matriz_google[1],
            "Google Routes — trânsito ao vivo e preditivo",
        )
    ...
    try:
        ...
        req = urllib.request.Request(
            url, headers={"User-Agent": "AproarLogisticsWeb/1.0"}
        )
        with urllib.request.urlopen(req, timeout=10) as response:
            res = json.loads(response.read())
            if res.get("code") == "Ok":
                distancias = [
                    [dist / 1000.0 for dist in row] for row in res["distances"]
                ]
                duracoes = [
                    [dur / 60.0 for dur in row] for row in res["durations"]
                ]
                return (
                    distancias,
                    duracoes,
                    "OSRM — malha viária sem trânsito ao vivo",
                )
    except:
        pass
    distancias, duracoes = [], []
    for i in range(len(coords)):
        ...
        for j in range(len(coords)):
            ...
            ), math.radians(coords[j][1] - coords[i][1])
            a = (
                math.sin(dLat / 2) ** 2
                + math.cos(math.radians(coords[i][0]))
                * math.cos(math.radians(coords[j][0]))
                * math.sin(dLon / 2) ** 2
            )
...
def pontuar_parada_rota(
    atual, ponto, unpicked, carrying, estrategia, get_dist_dur
):
    ...
    if is_dropoff and pendentes_mesmo_destino:
        urgencia_pendente = max(
            [peso_valido(t) for t in pendentes_mesmo_destino] + [1.0]
        )
        ...
        if not entrega_urgente_isolada:
            ...
            penalidade_retorno = max(
                8.0, min(ciclos_de_retorno or [8.0]) * 1.35
            )
    ...
    if "Menor Distância" in estrategia:
        ...
        score = (
            custo_deslocamento / max(prioridade, 0.001)
        ) + penalidade_retorno
...
def otimizar_sequencia_rota(
    ...
    if total_tarefas > 24:
        ...
        for _ in range(total_tarefas * 2 + 5):
            candidatos = {t["Origem"] for t in pendentes} | {
                t["Destino"] for t in no_carro
            }
            if not candidatos:
                break
            proximo = min(
                candidatos,
                key=lambda p: pontuar_parada_rota(
                    atual, p, pendentes, no_carro, estrategia, get_dist_dur
                )[0],
            )
    ...
    tempos_coleta = [
        numero_seguro(t.get("Tempo_Coleta", 10), 10.0) for t in tarefas
    ]
    tempos_entrega = [
        numero_seguro(t.get("Tempo_Entrega", 10), 10.0) for t in tarefas
    ]
    ...
    for _ in range(max_passos):
        ...
        for estado in estados:
            ...
            for ponto in pontos_disponiveis(
                estado["coletadas"], estado["entregues"]
            ):
                distancia, duracao = get_dist_dur(estado["atual"], ponto)

                ids_entrega = [
                    i
                    for i in tarefas_por_destino.get(ponto, [])
                    if (estado["coletadas"] & (1 << i))
                    and not (estado["entregues"] & (1 << i))
                ]
                ids_coleta = [
                    i
                    for i in tarefas_por_origem.get(ponto, [])
                ...
                tempo_servico = sum(
                    tempos_coleta[i] for i in ids_coleta
                ) + sum(tempos_entrega[i] for i in ids_entrega)
                _, nova_hora = avancar_relogio(
                    estado["hora"], duracao, tempo_servico
                )
                ...
                if ids_entrega and ainda_falta_para_o_ponto:
                    ...
                    urgente_depois = max(
                        [pesos[i] for i in ainda_falta_para_o_ponto] + [1]
                    )
                    excecao_urgente = (
                        "Urgências" in estrategia
                        and urgente_agora >= 4
                        and urgente_agora > urgente_depois
                    )
                ...
                carga_apos = (
                    novas_coletadas & ~novas_entregues
                ).bit_count()
                if "Descarregar" in estrategia:
                    incremento += (
                        carga_apos * (duracao + tempo_servico) * 0.55
                    )
                elif "Equilibrada" in estrategia:
                    incremento += (
                        carga_apos * (duracao + tempo_servico) * 0.05
                    )
        ...
        candidatos_ordenados = sorted(
            ...
            key=lambda e: e["custo"]
            + heuristica_restante(
                e["atual"], e["coletadas"], e["entregues"]
            ),
        )
    ...
    if not concluidos:
        concluidos = [
            e for e in estados if e["entregues"] == mascara_total
        ]
    if concluidos:
        ...
        for _ in range(2):
            ...
            while indice < len(polyline):
                ...
                if byte < 0x20:
                    break
            valores.append(
                ~(resultado >> 1) if resultado & 1 else resultado >> 1
            )
        latitude += valores[0]
        longitude += valores[1]
...
def buscar_geometria_google_trafego(coords_limpas, horario_partida=None):
    ...
    payload = {
        ...
        "intermediates": [
            waypoint(c, parada=True) for c in coords_limpas[1:-1]
        ],
        "travelMode": "DRIVE",
        "routingPreference": "TRAFFIC_AWARE_OPTIMAL",
        "trafficModel": "BEST_GUESS",
        "departureTime": partida.astimezone(timezone.utc)
        .isoformat()
        .replace("+00:00", "Z"),
    ...
    try:
        ...
        encoded = (
            rotas[0].get("polyline", {}).get("encodedPolyline", "")
            if rotas
            else ""
        )
...
def buscar_geometria_rota(coords_ordenadas, horario_partida=None):
    ...
    geometria_tomtom = buscar_geometria_tomtom_trafego(
        coords_limpas, horario_partida
    )
    if geometria_tomtom:
        return geometria_tomtom, True

    geometria_google = buscar_geometria_google_trafego(
        coords_limpas, horario_partida
    )
    if geometria_google:
    ...
    try:
        ...
        if res.get("code") == "Ok" and res.get("routes"):
            return [
                [lat, lon]
                for lat, lon in res["routes"][0]["geometry"]["coordinates"]
            ], True
    except:
        pass
    return [[lat, lon] for lat, lon in coords_limpas], False
...
def extrair_dados_completos(texto, card_name):
    num_match = re.search(
        r"\b(\d{4}(?:\.\d+)?|APR[A-Z0-9]+)\b", card_name, re.IGNORECASE
    )
    ...
    if texto:
        ...
        if (
            "TRANSBORDO" in texto_limpo.upper()
            or "TRANSBORDOS" in texto_limpo.upper()
        ):
            if unidade:
                origem = unidade
                destino = "ESCRITÓRIO"
            ...
            mo = re.search(
                r"(?i)(?:coletar|pegar|retirar|buscar|coleta)\s+(?:no|na|em|o|a|ao|à|aos|às)?\s*([^\:\n\.\-]+)",
                texto_limpo,
            )
            if mo:
                origem = normalizar_local(mo.group(1))

            md = re.search(
                r"(?i)(?:levar|entreg(?:ar|a)|devolver|encaminhar|transportar|deixar|entrega)\s+(?:para|no|na|em|o|a|ao|à|aos|às)?\s*([^\:\n\.]+)",
                texto_limpo,
            )
            if md:
                destino = normalizar_local(md.group(1))
...
def encontrar_endereco_na_descricao(descricao):
    ...
    if mo_link:
        return mo_link.group(1)
    mo_end = re.search(
        r"(?i)(?:endere[çc]o|local)\s*(?:\:|-)\s*([^\n]+)", descricao
    )
    if mo_end:
        return mo_end.group(1).strip()
    return None
...
def encontrar_conclusao_de_hoje(card_id, acoes):
    ...
    for acao in acoes:
        if acao.get("type") != "updateCard":
            continue
        dados = acao.get("data", {})
        if dados.get("card", {}).get("id") != card_id:
            continue
        if not lista_esta_concluida(
            dados.get("listAfter", {}).get("name", "")
        ):
            continue
        if lista_esta_concluida(
            dados.get("listBefore", {}).get("name", "")
        ):
            continue
        try:
...
def sincronizar_demandas(manual=False, forcar=False):
    ...
    trello_lists = {
        l["id"]: l["name"] for l in data.get("lists", []) if not l.get("closed")
    }
    demandas_extraidas = []

    for c in data.get("cards", []):
        if c.get("closed") or lista_esta_concluida(
            trello_lists.get(c.get("idList", ""), "").upper()
        ):
            continue
        short_name, origem, destino, materiais = extrair_dados_completos(
            c.get("desc", ""), c.get("name", "")
        )
        ...
        if endereco_card:
            ...
            if lat:
                if (
                    origem
                    and origem not in UNIDADES_PROPRIAS
                    and not fetch_one(
                        "SELECT lat FROM locais WHERE apelido = :apelido",
                        {"apelido": origem},
                    )
                ):
                    execute_db(
                        "INSERT INTO locais (apelido, endereco, lat, lon)"
                        " VALUES (:apelido, :end, :lat, :lon) ON CONFLICT"
                        " (apelido) DO UPDATE SET endereco=EXCLUDED.endereco,"
                        " lat=EXCLUDED.lat, lon=EXCLUDED.lon",
                        {
                            "apelido": origem,
                            "end": endereco_card,
                ...
                if (
                    destino
                    and destino not in UNIDADES_PROPRIAS
                    and not fetch_one(
                        "SELECT lat FROM locais WHERE apelido = :apelido",
                        {"apelido": destino},
                    )
        ...
        te_val = 10
        if (
            not st.session_state.demandas.empty
            and c["id"] in st.session_state.demandas["id"].values
        ):
            linha_antiga = st.session_state.demandas[
                st.session_state.demandas["id"] == c["id"]
            ].iloc[0]
            tc_val, te_val = (
                linha_antiga["Tempo_Coleta"],
                linha_antiga["Tempo_Entrega"],
            )
        ...
        demandas_extraidas.append(
            {
                ...
                "Supervisor": SUPERVISORES_MAP.get(
                    destino, "Sede / Logística"
                ),
            }
        )

    st.session_state.demandas = pd.DataFrame(
        demandas_extraidas, columns=COLUNAS_DEMANDAS
    )
...
def _escolher_campo(campos, palavras):
    ...
    for campo in campos:
        identificador = (
            f"{campo.get('name', '')} {campo.get('id', '')}".lower()
        )
...
def _montar_formulario_login(html, usuario, senha):
    ...
    campo_usuario = _escolher_campo(
        ...
        ("usu", "user", "login", "email"),
    )
    campo_senha = _escolher_campo(
        [
            c
            for c in campos_com_nome
            if c.get("type", "").lower() == "password"
        ],
        ("senha", "password", "pass"),
    )
    if not campo_usuario or not campo_senha:
        raise RuntimeError(
            "Não foi possível identificar os campos de acesso do portal."
        )
    dados = {
        c["name"]: c.get("value", "")
        for c in campos_com_nome
    ...
    botao = _escolher_campo(
        [
            c
            for c in campos_com_nome
            if c.get("type", "").lower() in ("submit", "button")
        ],
        ("entr", "acess", "login", "logar"),
    )
    if botao:
    ...
    if imagens:
        dados[f"{imagens[0]['name']}.x"] = "10"
        dados[f"{imagens[0]['name']}.y"] = "10"
...
def _parsear_resposta_rastreador(texto):
    ...
    for registro in texto.replace("\r", "").split(";"):
        ...
        if placa.startswith("TIF"):
            placa = "TIF-2123"
        elif placa.startswith("OSC"):
            placa = "OSC-3842"
...
def consultar_posicoes_protege(sessao, pagina_atual, veiculos):
    ...
    if not posicoes:
        raise RuntimeError(
            "O portal não devolveu posições. A sessão pode ter expirado."
        )
    return posicoes
    ...
    for login_url in RASTREADOR_LOGIN_URLS:
        try:
            ...
            resposta_login = sessao.post(
                url_post, data=dados, timeout=20, allow_redirects=True
            )
            ...
            return (
                sessao,
                pagina_atual,
                consultar_posicoes_protege(sessao, pagina_atual, veiculos),
            )
...
def carregar_config_protege():
    try:
        ...
        usuario, senha, veiculos = (
            str(config.get("usuario", "")).strip(),
            str(config.get("senha", "")).strip(),
            config.get("veiculos", RASTREADOR_VEICULOS_PADRAO),
        )
        return (
            usuario,
            senha,
            ",".join(str(v).strip() for v in veiculos)
            if isinstance(veiculos, (list, tuple))
            else str(veiculos).strip(),
        )
    except:
...
def loop_automacoes_background():
        ...
        if data:
            ...
            for c in cards:
                ...
                if lista_esta_concluida(
                    ...
                    momento_conclusao = encontrar_conclusao_de_hoje(
                        c["id"], acoes
                    )
                    if (
                        momento_conclusao
                        and str(c["id"]) not in ids_ja_notificados
                        and momento_conclusao.strftime("%d/%m/%Y")
                        == DATA_HOJE_REAL_STR
                    ):
                        short_name, origem, destino, materiais = (
                        ...
                        if (
                            agora_loop - momento_conclusao
                        ).total_seconds() / 60 <= 5 and url_webhook:
                            disparar_teams(
                                url_webhook,
                                f"✅ Entrega concluída — {destino}",
                                "✅ **Os materiais foram entregues na obra e a"
                                " demanda tomou baixa no"
                                f" Trello.**\n\n**Obra:** {short_name}\n\n**Local:**"
                                f" {destino}\n\n**Materiais:**"
                                f" {materiais}\n\n**Data e Hora:**"
                                f" {momento_conclusao.strftime('%d/%m/%Y às %H:%M')}",
                            )

                        execute_db(
                            "INSERT INTO historico_concluidos (id, obra,"
                            " origem, destino, materiais, data_conclusao,"
                            " hora_conclusao) VALUES (:id, :obra, :origem,"
                            " :destino, :mat, :data, :hora) ON CONFLICT (id) DO"
                            " UPDATE SET"
            ...
            if novas_entregas > 0:
                st.toast(
                    f"🔔 {novas_entregas} nova(s) baixa(s) no Trello"
                    " registrada(s)!",
                    icon="✅",
                )
    except:
    ...
    try:
        sessao, pagina = st.session_state.get(
            "protege_sessao"
        ...
        if sessao and pagina and ids_veiculos:
            ...
            for p in posicoes:
                if p["Velocidade (km/h)"] > 0:
                    dist_base_km = calcular_distancia_km(
                        lat_base, lon_base, p["Latitude"], p["Longitude"]
                    )
                    if dist_base_km > 0.5:
                        if not fetch_one(
                            "SELECT hora_inicio FROM inicio_movimento WHERE"
                            " placa=:placa AND data=:data",
                            {
                                "placa": p["Placa"],
                                "data": DATA_HOJE_REAL_STR,
                            },
                            ...
                            match_time = re.search(
                                r"(\d{2}:\d{2})", p["Última atualização"]
                            )
                            execute_db(
                                "INSERT INTO inicio_movimento (placa, data,"
                                " hora_inicio) VALUES (:placa, :data, :hora)"
                                " ON CONFLICT (placa, data) DO NOTHING",
                                {
                                    "placa": p["Placa"],
                                    "data": DATA_HOJE_REAL_STR,
                                    "hora": match_time.group(1)
                                    if match_time
                                    else agora_loop.strftime("%H:%M"),
                                },
                            )
            ...
            try:
                res_rota = fetch_one(
                    "SELECT json_locais FROM rota_ativa WHERE id=1 AND"
                    " data_rota=:data",
                    {"data": DATA_HOJE_REAL_STR},
                )
                if res_rota and posicoes:
                    ...
                    for p in posicoes:
                        ...
                        for nome_loc, coords in locais_rota.items():
                            if nome_loc == "ESCRITÓRIO":
                                continue
                            dist_km = calcular_distancia_km(
                                coords[0], coords[1], lat_v, lon_v
                            )
                        ...
                        parada_ativa = fetch_one(
                            "SELECT id, local, hora_chegada FROM"
                            " rastreio_paradas WHERE data=:data AND"
                            " placa=:placa AND hora_saida IS NULL ORDER BY id"
                            " DESC LIMIT 1",
                            {
                                "data": DATA_HOJE_REAL_STR,
                                "placa": placa_v,
                        ...
                        if local_proximo and vel_v < 5:
                            if parada_ativa:
                                if parada_ativa[1] != local_proximo:
                                    execute_db(
                                        "UPDATE rastreio_paradas SET"
                                        " hora_saida=:hora WHERE id=:id",
                                        {
                                            "hora": agora_hm,
                                            "id": parada_ativa[0],
                                        },
                                    ...
                                    execute_db(
                                        "INSERT INTO rastreio_paradas (data,"
                                        " placa, local, hora_chegada) VALUES"
                                        " (:data, :placa, :local, :hora)",
                                        {
                                            "data": DATA_HOJE_REAL_STR,
                                            "placa": placa_v,
                                            "local": local_proximo,
                                ...
                                execute_db(
                                    "INSERT INTO rastreio_paradas (data, placa,"
                                    " local, hora_chegada) VALUES (:data,"
                                    " :placa, :local, :hora)",
                                    {
                                        "data": DATA_HOJE_REAL_STR,
                                        "placa": placa_v,
...
try:
    ...
    st.markdown(
        ...
        ' margin-bottom: 25px; margin-top: -20px;"><img'
        f' src="data:image/png;base64,{encoded_string}" width="260"'
        ' style="flex-shrink: 0;"><h1 style="margin: 0; padding: 0;'
        ' line-height: 1.2; color: #e4e8f4;">TORRE DE CONTROLE'
        " LOGÍSTICO</h1></div>",
        unsafe_allow_html=True,
    )
except:
...
with st.sidebar:
    ...
    if hasattr(st, "fragment"):
        ...
        def _loop_operacoes():
            ...
            if (
                time.time() - st.session_state.get("ultima_sincronizacao", 0)
                > INTERVALO_TRELLO_SEGUNDOS
            ):
                sincronizar_demandas()
    ...
    def controles_planejamento_rota():
        ...
        st.selectbox(
            "🏁 Ponto de Saída",
            ["ESCRITÓRIO", "CASA DA INDÚSTRIA", "SENAI CENTRO", "MARACANAÚ"],
            key="cfg_ponto_saida",
        )
        estrategia_atual = st.selectbox(
            "🎯 Estratégia da Rota",
            [
                "⚖️ Equilibrada",
                "🏢 Foco em Descarregar",
            ...
            key="cfg_estrategia_rota",
        )
        st.caption(
            "ℹ️"
            f" *{ {'⚖️ Equilibrada': 'Mescla urgência com proximidade para fazer a rota mais lógica e eficiente.', '🏢 Foco em Descarregar': 'Prioriza entregar os materiais o quanto antes para esvaziar a caçamba.', '⛽ Menor Distância': 'Foca 100% no menor KM percorrido (Economia de combustível).', '🚨 Priorizar Urgências': 'Foca 100% nas demandas Vencidas ou programadas para Hoje.'}[estrategia_atual] }*"
        )
        st.checkbox(
            "Retornar à base no fim do dia", value=True, key="cfg_retornar_base"
        )

    controles_planejamento_rota()

veiculo_selecionado = st.session_state.get(
    "cfg_veiculo_rota", "Frota da Empresa (Calcula Gasolina)"
)
...
if st.session_state.demandas.empty:
    st.info(
        "👋 Bem-vindo(a) à Torre de Controle! Clique no botão **'🔄"
        " Sincronizar Manualmente'** no menu lateral para puxar as demandas ao"
        " vivo e começar."
    )

tab_roteiro, tab_rastreador, tab_demandas, tab_historico, tab_enderecos, tab_custos, tab_registros = (
    st.tabs(
        [
            "🗺️ Roteiro do Davi",
            "📡 Rastreador ao Vivo",
...
with tab_rastreador:
    ...
    st.caption(
        "Posições consultadas diretamente no portal. Atualização automática a"
        " cada 30 segundos."
    )
            ...
            try:
                ...
                if not sessao or not pagina:
                    sessao, pagina, posicoes = autenticar_protege(
                        usuario_protege, senha_protege, ids_veiculos
                    )
                    (
                        st.session_state["protege_sessao"],
                        st.session_state["protege_pagina"],
                    ) = (sessao, pagina)
                else:
                    try:
                        ...
                        sessao, pagina, posicoes = autenticar_proteSoy una IA basada en texto y no tengo esa capacidad.
