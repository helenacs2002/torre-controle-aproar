...
def aplicar_estilo_customizado():
    ...
    <style>
        ...
        button[kind="primary"], [data-testid="baseButton-primary"] { background-color: #ffffff !important; color: #080b1a !important; border-radius: 5px !important; font-weight: 700 !important; }
        button[kind="primary"]:hover { background-color: #e4e8f4 !important; transform: translateY(-1px); }
...
def aplicar_tempos_dinamicos(route_steps, dict_concluidos, start_time_str):
    ...
    for step in route_steps:
        ...
        if not has_pending and concluded_times:
            ...
            # Efeito cascata de atraso (Se a previsão no passado for menor que a hora atual, atualiza pra AGORA)
...
TEAMS_GRUPOS_UNIDADE = [("COMPRAS X ORÇAMENTOS", "geral_logistica", "Geral / Logística"), ("MARACANAÚ X COMPRAS", "maracanau", "MARACANAÚ"), ("HORIZONTE X COMPRAS", "horizonte", "HORIZONTE"), ("BARRA X COMPRAS", "barra", "BARRA"), ("CASA DA INDÚSTRIA X COMPRAS", "casa_industria", "CASA DA INDÚSTRIA"), ("CENTRO X COMPRAS", "centro", "CENTRO"), ("SEBRAE X COMPRAS", "sebrae", "SEBRAE"), ("MUSEU DA IND X COMPRAS", "museu", "MUSEU"), ("UNIFOR X COMPRAS", "unifor", "UNIFOR"), ("SEDE/PARANGABA X COMPRAS", "sede_parangaba", "PARANGABA")]
...
with tab_roteiro:
    ...
    # =========================================================================
    # O NOVO MOTOR DE ROTEAMENTO (MID-ROUTE TSP)
    # =========================================================================
    if st.button("🚀 Calcular Rota Otimizada / Atualizar Rota", type="primary", disabled=df_ativos.empty):
        with st.spinner("Analisando histórico e traçando rota a partir de onde o motorista está..."):
            ...
            # Puxa o histórico de concluídos pra saber o que já ficou pra trás
            ...
            # Reconstrói as paradas que ele já fez hoje
            past_route_steps = []
            current_point = ponto_saida
            current_time_tsp = INICIO_ROTA_MIN
            ...
            if rota_salva and len(dict_concluidos_torre) > 0:
                ...
                # Atualiza o ponto de saída do algoritmo para o ONDE O DAVI ESTÁ AGORA!
                if past_route_steps:
            ...
            # Deixa pra roteirizar apenas o que AINDA NÃO FOI ENTREGUE
            unpicked = [t for t in df_ativos.to_dict('records') if str(t['id']) not in dict_concluidos_torre]
            
            # Coleta as coordenadas necessárias (passado + futuro)
            ...
            # Se tivermos histórico, temos que calcular os km passados também
            for p_step in past_route_steps:
                total_km += p_step.get('dist', 0.0)
            ...
            while unpicked or carrying:
                ...
                # Se a gente tá criando a primeira parada da manhã do zero (sem histórico)
                ...
                chegada_str, saida_str, tempo_local_exibicao = ("07:00", "07:30", 30) if is_start_load else (format_time(current_time), format_time(current_time + service_mins), service_mins)
                if is_start_load: service_mins = 0
    ...
    # =========================================================================
    # RENDERIZAÇÃO DA INTERFACE DA ROTA NA TORRE DE CONTROLE
    # =========================================================================
    if st.session_state.get('rota_gerada', False):
        ...
        if st.session_state.get('demandas_adiadas'): st.warning(f"⚠️ **Capacidade Atingida:** {len(st.session_state['demandas_adiadas'])} demanda(s) com prazo folgado foi(ram) deixada(s) para amanhã para garantir que o Davi termine até as 17:00.")
        
        conn = sqlite3.connect(DB_FILE)
        cfg = pd.read_sql_query("SELECT consumo, preco_gasolina FROM config_frota WHERE id=1", conn).iloc[0]
        custo_rota = (total_km / float(cfg['consumo'])) * float(cfg['preco_gasolina']) if "Empresa" in veiculo_selecionado else total_km * 1.50
        
        # Puxa o histórico de concluídos e o horário da ignição do Rastreador para calcular o ETA dinâmico
        df_torre = pd.read_sql_query("SELECT id, hora_conclusao FROM historico_concluidos WHERE data_conclusao = ?", conn, params=(DATA_HOJE_REAL_STR,))
        dict_concluidos_torre = dict(zip(df_torre['id'].astype(str), df_torre['hora_conclusao']))
        res_inicio = conn.execute("SELECT MIN(hora_inicio) FROM inicio_movimento WHERE data=?", (DATA_HOJE_REAL_STR,)).fetchone()
        hora_inicio_real = res_inicio[0] if res_inicio and res_inicio[0] else "07:30"
        ...
        # O NÚCLEO DA SOLUÇÃO 2: Aplica os tempos reais na rota que vai ser desenhada
        ...
        with col_esq:
            ...
            for i, step in enumerate(route_steps):
                if step['type'] == 'lunch':
                    st.warning(f"🍔 **Pausa para Almoço** (Previsão de Parada: {step['dyn_chegada']} às {step['dyn_saida']})")
            ...
            st.success(f"🛣️ **Total Rodado Planejado:** {total_km:.1f} km | 💰 **{ 'Estimativa Gasolina' if 'Empresa' in veiculo_selecionado else 'Custo do Frete' }:** R$ {custo_rota:.2f}")
